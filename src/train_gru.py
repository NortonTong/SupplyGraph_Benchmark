...
def load_baseline(
    ...
    path = (
        ...
        / "baseline"
        / "gru"
        / f"gru_sequence_h{horizon}_L{window}_{temporal_type}.parquet"
    )
...
def main():
    ...
    if summaries:
        ...
        horizon_str = "_".join(str(h) for h in sorted({s["horizon"] for s in summaries}))
        transform_set = sorted({s["output_transform"] for s in summaries})
        transform_str = "_".join(transform_set)

        out_path = (
            out_dir
            / f"summary_gru_baseline2_h{horizon_str}_windows_{window_str}_outputs_{transform_str}.csv"
        )
