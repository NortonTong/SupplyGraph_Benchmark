...
class TGCNSeqDataset(Dataset):
    def __init__(self, X, Y, splits, seq_len, pre_len, split_name):
        ...
        X: [T, N, F]
        Y: [T, N]
        splits: np.array[str] shape [T] ('train' / 'val' / 'test')
        ...
        for t in range(T - seq_len - pre_len + 1):
            # dùng split của day cuối cùng của target
    ...
    def __getitem__(self, idx):
        ...
        t1 = t0 + self.seq_len
        t2 = t1 + self.pre_len
        x = self.X[t0:t1]      # [L, N, F]
        y = self.Y[t1:t2]      # [pre_len, N]
        return x, y
...
def build_tgcn_datasets(horizon: int, edge_type: str, seq_len: int, pre_len: int):
    pkg = load_base_pkg()
    X = pkg["X"]          # [T, N, F]
    if horizon == 1:
        Y = pkg["Y_h1"]   # [T, N]
    ...
    days = pkg["days"]
    splits = pkg["split"]   # np.array[str] [T]
    ...
    return train_ds, val_ds, test_ds, edge_index, feature_cols, pkg
...
def predict_and_save_tgcn(model, loader, edge_index, horizon, variant_tag, pkg, seq_len, pre_len):
    ...
    # cuối file train_tgcn.py
def main():
    # bạn có thể tune seq_len / pre_len
    seq_len = 14
    # pre_len=1 cho H=1, pre_len=7 cho H=7
    for h in [1, 7]:
        pre_len = 1 if h == 1 else 7

        train_ds, val_ds, test_ds, edge_index, feature_cols, pkg = build_tgcn_datasets(
            horizon=h,
            edge_type="plant",
            seq_len=seq_len,
            pre_len=pre_len,
        ...
        for epoch in range(1, 101):
            train_loss = train_epoch_tgcn(model, train_loader, edge_index_dev, optimizer)
            val_mae, val_rmse = eval_epoch_tgcn(model, val_loader, edge_index_dev)
            if val_rmse < best_rmse:
                best_rmse = val_rmse
                best_state = model.state_dict()
            if epoch % 10 == 0:
                print(f"[TGCN H{h}] Epoch {epoch} loss={train_loss:.4f} val_mae={val_mae:.4f} val_rmse={val_rmse:.4f}")

        if best_state is not None:
            model.load_state_dict(best_state)
        test_mae, test_rmse = eval_epoch_tgcn(model, test_loader, edge_index_dev)
        print(f"[TGCN H{h}] Test MAE={test_mae:.4f} RMSE={test_rmse:.4f}")
