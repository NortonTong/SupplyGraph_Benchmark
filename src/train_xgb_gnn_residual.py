...
def train_residual_projected(
    ...
    print(
        ...
        f"[H{horizon}][lag{lag_window}][{temporal_type}][view={edge_view}][{mode_name}][seed={seed}] ==="
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"[Residual-Proj-{edge_view}][{mode_name}][seed={seed}] Epoch {epoch:03d} | "
            f"train_loss={train_loss:.4f} | val_loss={val_loss:.4f}"
        )
        if early_stopper.step(val_loss, model):
            print(f"[Residual-Proj-{edge_view}][{mode_name}][seed={seed}] Early stopping at epoch {epoch:03d}")
            break
    ...
    tag = (
        f"baseline6_residual_proj_{edge_view}_"
        f"h{horizon}_lag{lag_window}_{temporal_type}_{mode_name}_seed{seed}"
    )
...
def train_residual_homo5(
    ...
    print(
        ...
        f"[H{horizon}][lag{lag_window}][{temporal_type}][{mode_name}][seed={seed}] ==="
    )
    ...
    nodeindex2local = {
        int(r["node_index"]): int(i) for i, (_, r) in enumerate(df_prod_nodes.iterrows())
    }
    offsets = {}
    offset = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss = iterate_days(idx_val, train_mode=False)
        print(
            f"[Residual-Homo5][{mode_name}][seed={seed}] Epoch {epoch:03d} | "
            f"train_loss={train_loss:.4f} | val_loss={val_loss:.4f}"
        )
        if early_stopper.step(val_loss, model):
            print(f"[Residual-Homo5][{mode_name}][seed={seed}] Early stopping at epoch {epoch:03d}")
            break
...
def train_residual_hetero5(
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss = iterate_days(idx_val, train_mode=False)
        print(
            f"[Residual-Hetero5][{mode_name}][seed={seed}] Epoch {epoch:03d} | "
            f"train_loss={train_loss:.4f} | val_loss={val_loss:.4f}"
        )
        if early_stopper.step(val_loss, model):
            print(f"[Residual-Hetero5][{mode_name}][seed={seed}] Early stopping at epoch {epoch:03d}")
            break
