...
Supply chain planning là một bài toán tự nhiên cho **graph**: sản phẩm, nhà máy, kho và nhóm sản phẩm tạo thành một mạng lưới với cấu trúc rõ ràng. Tuy nhiên, nghiên cứu về graph‑based supply chain forecasting vẫn còn mới và thiếu một benchmark chuẩn, có thể reproduce đầy đủ. [github](https://github.com/ciol-researchlab/SupplyGraph)
...
- Data và code không hoàn toàn mở, dẫn tới việc tái lập đầy đủ pipeline rất khó. [arxiv](https://arxiv.org/html/2401.15299v1)
...
- Các công trình sau sử dụng SupplyGraph chưa thống nhất về pipeline, tạo ra một “khoảng trống” về benchmark chuẩn cho GNN trong supply chain. [themoonlight](https://www.themoonlight.io/en/review/graph-neural-networks-in-supply-chain-analytics-and-optimization-concepts-perspectives-dataset-and-benchmarks)
...
- Một pipeline preprocessing **minh bạch, có code đầy đủ**, từ raw CSV đến `.parquet` và `.pt` cho từng baseline/GNN, với schema rõ ràng và được mô tả chi tiết bằng markdown. [github](https://github.com/ciol-researchlab/SupplyGraph)
...
- Phân tích phân phối target `sales_order` (unit), thiết kế **output parameterizations** (raw vs softplus vs log1p) cho GRU và GNN phù hợp với phân phối **zero‑inflated + heavy‑tailed** của demand. [sciencedirect](https://www.sciencedirect.com/science/article/pii/S0952197625003392)
...
Dataset giữ đúng “spirit” của SupplyGraph: **nodes = products**, edges = quan hệ giữa products; các series thời gian là các luồng quantity theo ngày. Ba thành phần chính: [arxiv](https://arxiv.org/html/2401.15299v1)
...
Kết quả: phân phối **zero‑inflated**, right‑skewed rất mạnh (heavy tail), mean lớn nhưng median thấp, tail dài về phía giá trị lớn – đặc trưng điển hình của intermittent demand. [nixtla](https://www.nixtla.io/blog/intermittent-demand)
...
- Edges: toàn bộ quan hệ product–(plant/group/subgroup/storage) và edges giữa non‑product nodes, nhưng **không phân biệt loại** (untyped).  
- Flatten:
...
- Không dùng lag/rolling: GNN nhìn snapshot feature tại từng day, kết hợp với cấu trúc graph để học context.  
...
- Right‑skewed mạnh, tail dài, nhiều outliers lớn. [juanitorduz.github](https://juanitorduz.github.io/zi_tsb_numpyro/)
...
- Tránh giá trị âm cho demand, nhưng vẫn dùng **loss MSE trên scale gốc**.  
- Áp dụng cho:
...
Các metric được báo cáo trên test: MAE, RMSE, MAPE, sMAPE – phù hợp với thực hành forecast demand zero-inflated. [sciencedirect](https://www.sciencedirect.com/science/article/pii/S0952197625003392)
...
- Điều này cho thấy chuỗi demand có mức tự tương quan đủ rõ để một chiến lược cực kỳ đơn giản vẫn đạt lỗi tương đối thấp theo sMAPE, và là baseline tham chiếu có ý nghĩa cho toàn bộ benchmark. [linkedin](https://www.linkedin.com/pulse/why-intermittent-demand-still-trips-us-up-what-do-badulescu-ph-d-twivc)
...
- Với lag 7, XGBoost tabular vẫn giữ hiệu năng cạnh tranh (MAE ≈ 409.7, RMSE ≈ 1035.7, sMAPE ≈ 127.3), cho thấy **lag-based gradient boosting với OHE categorical** là baseline khó bị vượt qua trong bối cảnh demand zero‑inflated, heavy‑tailed. [nixtla](https://www.nixtla.io/blog/intermittent-demand)
...
- Kết quả này phù hợp với kinh nghiệm thực tế: trên dữ liệu vừa và nhiều zero, **sequence model đơn giản (GRU) với MSE point loss không nhất thiết vượt được XGBoost lag‑based** nếu không có tuning sâu hoặc loss chuyên biệt cho intermittent demand. [juanitorduz.github](https://juanitorduz.github.io/zi_tsb_numpyro/)
...
- **Graph information hữu ích nhất khi kết hợp cẩn thận**: trong các thử nghiệm hiện tại, việc encode graph vào features cho XGBoost mang lại lợi ích rõ ràng hơn so với GNN end‑to‑end với MSE point loss, nhấn mạnh tầm quan trọng của hybrid design (graph + mạnh tabular). [github](https://github.com/ciol-researchlab/SupplyGraph)
- **Kiến trúc GNN cần đi kèm loss phù hợp với zero‑inflated demand**: các hiện tượng loss bất ổn với Softplus/log1p cho thấy rằng để khai thác graph triệt để, cần những thiết kế chuyên biệt (ví dụ hurdle/mixture likelihood, zero‑inflated count models) thay vì chỉ thay backbone bằng GNN với MSE cổ điển. [juanitorduz.github](https://juanitorduz.github.io/zi_tsb_numpyro/)
